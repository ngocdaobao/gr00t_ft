...
class Gr00tN1d7ActionHead_history(Gr00tN1d7ActionHead):
    def __init__(self, window_size: int, config: Gr00tN1d7Config):
        # Initialize parent with config and set up history cache
        super().__init__(config)
        self.window_size = window_size
        # history_cache holds lists of tensors for state, obs and their masks
        self.history_cache = {"state": [], 
                              "obs": [], 
                              "obs_mask": [], 
                              "image_mask": [], 
    ...
    def forward(self, backbone_output: BatchFeature, action_input: BatchFeature) -> BatchFeature:
        ...
        Args:
            window_size: Size of the history window
            backbone_output: Output from the backbone model containing:
                - backbone_features: [B, seq_len, backbone_embedding_dim]
                - backbone_attention_mask: [B, seq_len]
            action_input: Input containing:
        ...
        # Set frozen modules to eval
        # logging.info(
        #     f"History cache sizes - state: {len(self.history_cache['state'])}, obs: {len(self.history_cache['obs'])}"
        # )
        self.set_frozen_modules_to_eval_mode()
        backbone_output = self.process_backbone_output(backbone_output)
        ...
        vl_embeds = backbone_output.backbone_features
        device = vl_embeds.device
        # logging.info(f"Backbone features shape: {vl_embeds.shape}")
        ...
        # Append to history and trim to window_size. Also store attention/image masks so
        # encoder keys/values and masks remain aligned when concatenated.
        # IMPORTANT: Detach cached tensors to break the computation graph and prevent
        # double-backward errors when these tensors are reused in subsequent forward passes.
        # store backbone attention mask and optional image mask
        self.history_cache["state"].append(state_features)
        self.history_cache["obs"].append(backbone_output.backbone_features)
        obs_mask = backbone_output.backbone_attention_mask
        self.history_cache["obs_mask"].append(obs_mask)
        if hasattr(backbone_output, "image_mask") and backbone_output.image_mask is not None:
            self.history_cache["image_mask"].append(backbone_output.image_mask)
        if hasattr(backbone_output, "backbone_attention_mask") and backbone_output.backbone_attention_mask is not None:
            self.history_cache["backbone_attention_mask"].append(backbone_output.backbone_attention_mask)
        ...
        # logging.info(f"State features after history concat shape: {state_features.shape}")
        # logging.info(f"VL embeds after history concat shape: {vl_embeds.shape}")
        # logging.info(f"VL attn mask shape: {vl_attn_mask.shape} image_mask: {None if image_mask is None else image_mask.shape}")
