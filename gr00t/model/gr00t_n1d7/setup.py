...
class Gr00tN1d7Pipeline(ModelPipeline):
    ...
    def _create_model(self):
        ...
        action_head_type = type(model.action_head).__name__
        print(f"[DEBUG] Model created with action_head type: {action_head_type}", flush=True)
        logging.info(f"Model created with action_head type: {action_head_type}")
        if get_rank() == 0:
