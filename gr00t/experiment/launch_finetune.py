...
if __name__ == "__main__":
    ...
    # If the base model path is a training checkpoint (for example checkpoint-20000),
    # treat ft_config.max_steps as additional steps to run from that checkpoint.
    checkpoint_match = re.search(r"checkpoint-(\d+)$", ft_config.base_model_path.rstrip("/"))
    if checkpoint_match is not None:
        resume_step = int(checkpoint_match.group(1))
        config.training.max_steps = resume_step + ft_config.max_steps
